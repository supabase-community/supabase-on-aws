...
const updateSecret = async (secretId: string) => {
  ...
  const newSecret = {
    ...secret,
    url: `postgres://${secret.username}:${secret.password}@${secret.host}:${secret.port}/${secret.dbname||'postgres'}`,
    url_auth: `postgres://${secret.username}:${secret.password}@${secret.host}:${secret.port}/${secret.dbname||'postgres'}?search_path=auth`,
  };
